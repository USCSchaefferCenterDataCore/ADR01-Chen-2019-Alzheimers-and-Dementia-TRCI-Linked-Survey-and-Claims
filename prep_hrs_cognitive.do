...
gen dementia = (cogstate == 1) if !missing(cogstate)

bys hhidpn (wave): egen firsttime = min(cond(cogstate == 1), wave, .)

gen dementiae = dementia
replace dementiae = 1 if wave >= firsttime & dementiae == 0
...
* "Confirmed" dementia - make sure subsequent wave has either dementia or CIND
gen dementiae_strict = dementia
bys hhidpn (wave): egen confirmed = min(cond(inlist(cogstate[_n+1],1,2) & cogstate == 1), wave, .)

* fill in the zeros if not yet confirmed
replace dementiae_strict = 0 if dementia == 1 & wave < confirmed
* fill forward once confirmed
replace dementiae_strict = 1 if dementiae_strict == 0 & wave >= confirmed
...
save "$outdata/cogstate_wide.dta", replace
